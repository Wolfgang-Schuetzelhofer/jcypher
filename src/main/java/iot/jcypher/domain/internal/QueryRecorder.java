...
import iot.jcypher.domain.internal.RecordedQuery.Invocation;
import iot.jcypher.domain.internal.RecordedQuery.Statement;
...
public class QueryRecorder {
	...
	public static void recordInvocation(Object on, String method, Object result, Object... params) {
		...
		for (int i = 0; i < params.length; i++) {
			...
			if (param instanceof Literal)
				...
				} else // assume it is a literal
					parameters.add(rqh.recordedQuery.literal(((PlaceHolder)param).value));
			}
		}
		rqh.recordInvocation(on, method, result, parameters);
	...
	private static List<Statement> adoptStatements(RecQueryHolder from,
		...
		for (Statement s : params) {
			if (s instanceof Invocation) {
				String or = ((Invocation)s).getOnObjectRef();
				...
				((Invocation) s).setOnObjectRef(id);
			}
		}
		return params;
	}
	...
	public static void recordInvocationReplace(DomainObjectMatch<?> on, Object toReplace) {
		...
		if (trqh != null) {
			for (Statement stmt : trqh.recordedQuery.getStatements()) {
				if (stmt instanceof Invocation) {
					...
					((Invocation)stmt).setOnObjectRef(onId);
				}
			}
	...
	public static void recordBracketOpen(AbstractDomainQuery on) {
		RecQueryHolder rqh = getRecQueryHolder(on);
		rqh.recordBracketOpen();
	}
	
	public static void recordBracketClose(AbstractDomainQuery on) {
		RecQueryHolder rqh = getRecQueryHolder(on);
		rqh.recordBracketClose();
	}
	...
	public static RecordedQuery getRecordedQuery(AbstractDomainQuery query) {
		return getRecQueryHolder(query).recordedQuery;
	}
	...
	private static class RecQueryHolder {
		...
		private void recordInvocation(Object on, String method, Object result, List<Statement> parameters) {
		...
		private void recordBracketOpen() {
			this.recordedQuery.addBracketOpen();
		}
		
		private void recordBracketClose() {
			this.recordedQuery.addBracketClose();;
		}
